...
export function getPiPendingForCounselor(
  ...
  const allInvoices = invoices || getFinance().invoices;
  ...
  return allInvoices
    ...
    .map(invoice => {
      ...
      return {
        ...
        openBalance: piOpenBalance(invoice),
...
export function findOpenPiForStudent(studentName: string, invoices?: Invoice[]): Invoice | null {
  const allInvoices = invoices || getFinance().invoices;
