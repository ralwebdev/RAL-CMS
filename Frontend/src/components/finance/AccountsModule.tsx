...
function BillingTab({ role }: { role: RoleScope }) {
  ...
  const visibleInvoices = useMemo(() => {
    ...
    // Executives see what they created OR all student admissions (since they are auto-generated by Admin)
    return fin.invoices.filter(i => 
      i.createdBy === myId || 
      i.revenueStream === "Student Admissions" ||
      dispatchByInv.get(i.id)?.generatedBy === myId
    );
...
function ExportsTab() {
  ...
  const preview = useMemo(() => buildVouchers(fin, txnType, range, from, to), [txnType, range, from, to, fin]);
