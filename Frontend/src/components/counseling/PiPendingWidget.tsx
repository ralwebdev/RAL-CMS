...
export function PiPendingWidget({ counselorId, studentNames, invoices }: Props) {
  useFinanceTick(); // re-render on finance store updates
  const [open, setOpen] = useState(false);
  const navigate = useNavigate();

  const rows = useMemo(
    () => getPiPendingForCounselor(counselorId, studentNames, invoices),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [counselorId, studentNames.join("|"), invoices],
  );
